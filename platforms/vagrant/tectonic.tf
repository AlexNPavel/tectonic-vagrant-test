module "bootkube" {
  ...
  kube_apiserver_url = "https://${var.tectonic_vagrant_controller_domain}:443"

  # Address of Identity service
  oidc_issuer_url = "https://${var.tectonic_vagrant_ingress_domain}/identity"
  ...
  etcd_endpoints = ["${split(",",
    length(compact(var.tectonic_etcd_servers)) == 0
      ? join(",", var.tectonic_vagrant_controller_domains)
  ...
  etcd_cert_dns_names = ["c1.vagrant"]

  experimental_enabled = "${var.tectonic_experimental}"

  master_count = "${length(var.tectonic_vagrant_controller_names)}"
}
...
module "tectonic" {
  ...
  base_address = "${var.tectonic_vagrant_ingress_domain}"
